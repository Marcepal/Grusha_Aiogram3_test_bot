...
async def fill_profile(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Давай начнем!пожалуйста введи свое имя",
...
async def incorrenct_form_sex(message: Message, state: FSMContext):
    await message.answer("Нажми на кнопку")

@router.message(Form.about)
async def form_about(message: Message, state: FSMContext):
    if len(message.text) < 9:
        await message.answer("слишком мало,напиши о себе больше")
    else:
